...
function getEscapeMode(filename: string): "html" | "none" {
  if (filename.endsWith(".html.tc")) return "html";
  return "none";
}
...
export function compile(options: CompileOptions): CompileResult {
  ...
  const typeName = ast.typeDirective?.typeName;
  const from = ast.typeDirective?.from;

  // Type checking
  let diagnostics: Diagnostic[] = [];
  if (options.typecheck && options.templatePath && typeName && from) {
    try {
      ...
      diagnostics = typecheck(ast, dataType, { templateDir });
    } catch (err) {
      diagnostics = [{
  ...
  function compilePartial(node: ASTNode): string {
    ...
    const importPath = node.path.replace(/\.tc$/, "");

    partialImports.push({ varName: partialVar, importPath });

    const args = node.dataExpr ? compileExpr(node.dataExpr) : "";
    return `${out} += ${partialVar}(${args});\n`;
  }
  ...
  function compileLayoutBlock(node: ASTNode): string {
    ...
    code += compileBody(node.body);
    outputVarStack.pop();
    const dataArg = node.dataExpr ? `${compileExpr(node.dataExpr)}, ` : "";
    code += `${currentOutputVar()} += ${layoutVar}(${dataArg}${contentVar});\n`;
  ...
  let code = `// Auto-generated by Typecek — do not edit\n`;
  ...
  if (escapeMode === "html") {
    code += `import { escapeHtml as __escapeHtml } from "@typecek/runtime";\n`;
  }
  ...
  code += `\n`;
  const dataParam = typeName ? `data: ${typeName}` : "";
  const contentParam = hasContentTag ? `__content: string = ""` : "";
  const params = [dataParam, contentParam].filter(Boolean).join(", ");
  code += `export default function render(${params}): string {\n`;
  code += `let __out = "";\n`;
  code += bodyCode;
  code += `return __out;\n`;
  code += `}\n`;
